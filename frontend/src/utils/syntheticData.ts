...
function clampedWalk(current: number, step: number, min: number, max: number): number {
...
export function generateSyntheticPayload(
  ...
  const anomalyTypes = ["SEMANTIC_LOOP", "IDLE_HOG", "MEMORY_LEAK"] as const;
...
export function generateBackfillPayloads(
  count: number,
  intervalSeconds: number = 2
): ChartTimelinePayload[] {
  const now = Math.floor(Date.now() / 1000);
  const result: ChartTimelinePayload[] = [];
  for (let i = count - 1; i >= 0; i--) {
    result.push(
      generateSyntheticPayload(now - i * intervalSeconds)
    );
  }
  return result;
}
