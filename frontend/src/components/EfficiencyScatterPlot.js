...
function drawScatter(ctx, state) {
    ...
    if (hoveredPts.length > 0) {
        ...
        ctx.fillText(`SM: ${pt.smUtilizationPct.toFixed(1)}%  VRAM: ${pt.vramAllocatedMb.toFixed(0)}MB`, tx + 8, ty + 34);
...
export function EfficiencyScatterPlot({ points, height = 280, }) {
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        const closest = pts.reduce((prev, curr) => Math.abs(curr.smUtilizationPct - smVal) <
