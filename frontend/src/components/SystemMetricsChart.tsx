...
interface SystemMetricsChartProps {
  buffer: ReturnType<typeof useSlidingTimeBuffer>;
  version: number;
  height?: number;
}
...
function drawChart(
  ...
  const cpuMax = Math.max(
    ...data.map((d) => d.seriesData.cpuUtilization),
    1
  );
  const memMax = Math.max(
    ...data.map((d) => d.seriesData.memoryUsageMb),
    1
  );
  const smMax = 100;
  const vramMax = Math.max(
    ...data.map((d) => d.seriesData.gpuVramAllocatedMb),
    1
  );
  ...
  function leftY(val: number): number {
    return padding.top + plotH - (val / leftMax) * plotH;
  }

  function rightY(val: number): number {
  ...
  if (ts !== null && ts >= tMin && ts <= tMax) {
    ...
    const closest = data.reduce((prev, curr) =>
      Math.abs(curr.timestamp - ts) < Math.abs(prev.timestamp - ts)
        ? curr
        : prev
    );
...
export function SystemMetricsChart({
  ...
  const dataRef = useRef<ChartTimelinePayload[]>([]);
